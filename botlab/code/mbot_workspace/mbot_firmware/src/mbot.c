...
// PID Filters
rc_filter_t wl_PD_filter;
rc_filter_t wr_PD_filter;

rc_filter_t wl_I_filter;
rc_filter_t wr_I_filter;

rc_filter_t w_PID_filter;

// Low Pass Filters
rc_filter_t v_accel_filter;
rc_filter_t w_accel_filter;

rc_filter_t el_LP_filter;
rc_filter_t er_LP_filter;

rc_filter_t pwml_LP_filter;
rc_filter_t pwmr_LP_filter;

rc_filter_t w_pid_LP_filter;
...
void mbot_vel_cmd_cb(serial_twist2D_t *msg)
{
    ...
    /* Choose the Open Loop (Feed Forward) or Closed Loop Here */
    // drive_mode = MODE_MBOT_VEL; // Open Loop
    drive_mode = MODE_MBOT_VEL_CL; // Closed Loop
}

void mbot_motor_vel_cmd_cb(serial_mbot_motor_vel_t *msg)
{
    memcpy(&mbot_motor_vel_cmd, msg, sizeof(serial_mbot_motor_vel_t));
    drive_mode = MODE_MOTOR_VEL_CL;
}
...
void mbot_read_encoders(serial_mbot_encoders_t* encoders){
    ...
    /* March Low Pass Filters on Encoder Data ----------------------------------------------------------------------------------------- */
    ...
    // encoders->delta_ticks[params.mot_left] = rc_filter_march(&el_LP_filter, mbot_encoder_read_delta(params.mot_left));
    // encoders->delta_ticks[params.mot_right] = rc_filter_march(&er_LP_filter, mbot_encoder_read_delta(params.mot_right));
    /* -------------------------------------------------------------------------------------------------------------------------------- */
...
void mbot_print_state(serial_mbot_imu_t imu, serial_mbot_encoders_t encoders, serial_pose2D_t simulated_odometry, serial_pose2D_t odometry, serial_mbot_motor_vel_t motor_vel){
    printf("\033[2J\r");
    ...
    const char* imu_acc_headings[] = {"ROLL", "PITCH", "YAW"};
    const char* enc_headings[] = {"ENC 0", "ENC 1", "ENC 2"};
    const char* odom_headings[] = {"X", "Y", "THETA"};
    const char* sim_odom_headings[] = {"X", "Y", "THETA"};
    ...
    // buf[0] = '\0';
    // float sim_odom_array[3] = {simulated_odometry.x, simulated_odometry.y, simulated_odometry.theta};
    // generateTableFloat(buf, 1, 3, "SIMULATED ODOMETRY", sim_odom_headings, sim_odom_array);
    // printf("\r%s", buf);
...
;
    ...
    mbot_calculate_motor_vel(mbot_encoders, &mbot_motor_vel); // Velocity of each motor based on Encoders

    if(MBOT_DRIVE_TYPE == DIFFERENTIAL_DRIVE){
        /* Rely on IMU Data for Body Velocity Calculations - Current Solution is Pure IMU Yaw */
    ...
    mbot_calculate_odometry(mbot_vel_dr, MAIN_LOOP_PERIOD, &mbot_odometry_dr);
    ...
    if (global_comms_status == COMMS_OK)
    {
        ...
        }else if(drive_mode == MODE_MBOT_VEL_CL){
            /* Closed Loop Controller - Input are Linear and Angular Velocity Commands */
            if(MBOT_DRIVE_TYPE == OMNI_120_DRIVE){/* Omni Not Implemented */}
            else if(MBOT_DRIVE_TYPE == DIFFERENTIAL_DRIVE){

                float v = mbot_vel_cmd.vx; // Linear Velocity
                float w = mbot_vel_cmd.wz; // Anguler Velocity

                /* Acceleration-Limiting Low Pass Filters for Commanded Velocities ----------------------- */
                v = rc_filter_march(&v_accel_filter, v);
                w = rc_filter_march(&w_accel_filter, w);
                /* --------------------------------------------------------------------------------------- */

                /* PID Controller for Angular Velocity --------------------------------------------------- */
                // float w_pid = rc_filter_march(&w_PID_filter, w - mbot_vel.wz);
                // w = rc_filter_march(&w_pid_LP_filter, w + w_pid); // reduce noise in adjustments
                /* --------------------------------------------------------------------------------------- */

                /* Compute Wheels' Angular Velocity ------------------------------------------------- */
                mbot_motor_vel_cmd.velocity[params.mot_left] = (v - DIFF_BASE_RADIUS * w) / DIFF_WHEEL_RADIUS;
                mbot_motor_vel_cmd.velocity[params.mot_right] = (-v - DIFF_BASE_RADIUS * w) / DIFF_WHEEL_RADIUS;
                /* --------------------------------------------------------------------------------------- */

                /* PID Control for Wheels' Angular Velocity ---------------------------------------------- */
                float vel_left_goal =  mbot_motor_vel_cmd.velocity[params.mot_left];
                float vel_right_goal = mbot_motor_vel_cmd.velocity[params.mot_right];

                // Use MBOT_VEL Channel to Obtain Data to Support Tuning; Set Values Here
                // mbot_vel.vy = mbot_odometry_dr.x;
                // mbot_vel.wz = mbot_odometry_dr.y;
                // mbot_vel.vx = mbot_odometry_dr.theta;
                mbot_vel.vy = mbot_motor_vel.velocity[params.mot_left]; /* Share the measured term */
                mbot_vel.wz = vel_left_goal; /* Share the reference term */
                // mbot_vel.vx = ???; /* Share other useful data */

                float measured_left = rc_filter_march(&el_LP_filter, mbot_motor_vel.velocity[params.mot_left]);
                float measured_right = rc_filter_march(&er_LP_filter, mbot_motor_vel.velocity[params.mot_right]);

                float wl_pid = rc_filter_march(&wl_PD_filter, vel_left_goal - measured_left);
                float wr_pid = rc_filter_march(&wr_PD_filter, vel_right_goal - measured_right);

                float wl_i_term = rc_filter_march(&wl_I_filter, vel_left_goal - measured_left) * (vel_left_goal * 0.001);
                float wr_i_term = rc_filter_march(&wr_I_filter, vel_right_goal - measured_right) * (vel_right_goal * 0.001);

                // Account for Integral Windup
                if (wl_i_term > 1.0) {
                    wl_i_term = 0;
                    rc_filter_reset(&wl_I_filter);
                }

                if (wr_i_term > 1.0) {
                    wr_i_term = 0;
                    rc_filter_reset(&wr_I_filter);
                }
                ...
                /* Combine Feedback into Feedforward ----------------------------------------------------- */
                float vel_left_comp = params.motor_polarity[params.mot_left] * mbot_motor_vel_cmd.velocity[params.mot_left];
                float vel_right_comp = params.motor_polarity[params.mot_right] * mbot_motor_vel_cmd.velocity[params.mot_right];

                mbot_motor_pwm.utime = global_utime;

                mbot_motor_pwm_cmd.pwm[params.mot_left] = rc_filter_march(&pwml_LP_filter,  _calibrated_pwm_from_vel_cmd(vel_left_comp + wl_pid, params.mot_left));
                mbot_motor_pwm_cmd.pwm[params.mot_right] = rc_filter_march(&pwmr_LP_filter, _calibrated_pwm_from_vel_cmd(vel_right_comp + wr_pid, params.mot_right));
                /* --------------------------------------------------------------------------------------- */
        ...
        // // Reset Filters
        // rc_filter_free(&v_accel_filter);
        // rc_filter_free(&w_accel_filter);
        // rc_filter_free(&wl_PD_filter);
        // rc_filter_free(&wr_PD_filter);
        // rc_filter_free(&wl_I_filter);
        // rc_filter_free(&wr_I_filter);
...
{   
    ...
    /* Initialize All the RC Filters */
    v_accel_filter = rc_filter_empty();
    w_accel_filter = rc_filter_empty();
    wl_PD_filter = rc_filter_empty();
    wr_PD_filter = rc_filter_empty();
    el_LP_filter = rc_filter_empty();
    er_LP_filter = rc_filter_empty();
    pwml_LP_filter = rc_filter_empty();
    pwmr_LP_filter = rc_filter_empty();
    w_pid_LP_filter = rc_filter_empty();
    w_PID_filter = rc_filter_empty();
    wl_I_filter = rc_filter_empty();
    wr_I_filter = rc_filter_empty();
     /* ---------------------------- */

    printf("Initialized Empty Filters\n");

    rc_filter_first_order_lowpass(&v_accel_filter, 0.02, 0.15); // 0.02 and 0.1
    rc_filter_first_order_lowpass(&w_accel_filter, 0.02, 0.07); // 0.02 and 0.05

    rc_filter_first_order_lowpass(&el_LP_filter, 0.02, 0.05);
    rc_filter_first_order_lowpass(&er_LP_filter, 0.02, 0.05);

    rc_filter_first_order_lowpass(&pwml_LP_filter, 0.02, 0.1);
    rc_filter_first_order_lowpass(&pwmr_LP_filter, 0.02, 0.1);

    rc_filter_first_order_lowpass(&w_pid_LP_filter, 0.02, 0.25);

    printf("Created Lowpass Filters\n");
    

    rc_filter_pid(&wl_PD_filter, 2.2, 0.0, 0.00002, 0.00066, 0.001); // I = 0.00037437
    rc_filter_pid(&wr_PD_filter, 2.2, 0.0, 0.00002, 0.00066, 0.001);

    rc_filter_pid(&wl_I_filter, 0.0, 0.00001, 0.0, 0.00066, 0.001);
    rc_filter_pid(&wr_I_filter, 0.0, 0.00001, 0.0, 0.00066, 0.001);

    /*

    rc_filter_pid(&wl_PD_filter, 2.05, 0.0, 0.015, 0.00066, 0.001); // I = 0.00037437
    rc_filter_pid(&wr_PD_filter, 2.05, 0.0, 0.015, 0.00066, 0.001);

    rc_filter_pid(&wl_I_filter, 0.0, 0.0005, 0.0, 0.00066, 0.001);
    rc_filter_pid(&wr_I_filter, 0.0, 0.0005, 0.0, 0.00066, 0.001);

    */

    rc_filter_pid(&w_PID_filter, 1.35, 0.0, 0.000015, 0.00066, 0.001); // P = 0.1 is good

    printf("Created PID Filters\n");
    ...
    while(running){
        ...
        // Print State
        mbot_print_state(mbot_imu, mbot_encoders, mbot_simulated_odometry, mbot_odometry, mbot_motor_vel);
        sleep_ms(200);  
        counter++;
    }
}